...
- Fixed `chart_trends::peaks` and `chart_trends::valleys` dropping a legitimately retained extremum when a stronger extremum appeared after a monotonic run. The single `last_peak_idx`/`last_valley_idx` tracker served as both the sliding proximity anchor and the pointer to the retained extremum; once the anchor slid forward through ignored neighbors, the dedup `pop` removed the wrong (still-valid) extremum. The replace step now pops only when the new extremum is within `closest_neighbor` of the retained one, otherwise it is recorded as a separate extremum (e.g. `peaks(&[110.0, 109.0, 108.0, 120.0], 2, 2)` now returns `[(110.0, 0), (120.0, 3)]` instead of `[(120.0, 3)]`).
